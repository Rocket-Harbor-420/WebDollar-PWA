...
- `connect(endpoint?)` (y alias `connectToMarketplace`): valida una fuente Mainnet. Si encuentra el capability document `webdollar-marketplace-v1`, habilita Assets y Marketplace; si identifica la API nativa de WebDollar2, habilita únicamente lectura de Assets mediante `/account` y `/asset`.
- `fetchAssets(address?)`: consulta WEBD y tokens reales. Para un endpoint Marketplace usa `/address/assets`; para WebDollar2 usa `/account?address=...`, toma los identificadores desde `accountsExtra[].asset` y consulta los metadatos de cada activo en `/asset?hash=...`; nunca lee almacenamiento de cartera.
- `listAssetForSale(assetId, amount, price)`: valida los campos y llama a `window.webdollarCore.signMarketplaceOrder(data)`. Si no hay un endpoint Marketplace compatible, abre la revisión humana y, tras confirmar, deja la orden firmada en `pending-network`; nunca la presenta como publicada.
- `getListings()`: obtiene ofertas activas desde `/marketplace/listings` únicamente cuando `marketplaceProtocolSupported` es verdadero; la API nativa de WebDollar2 no se interpreta como un libro de órdenes.
...
La confirmación final llama internamente a `WalletCore.signMarketplaceOrder(data, {confirmed:true})`. El Core comprueba que la cartera esté desbloqueada, valida Asset ID, cantidad, precio, vendedor y listado, y firma una orden canónica Ed25519. El módulo entrega los bytes de la orden firmada al endpoint Mainnet y solo marca la operación aceptada cuando el nodo devuelve una respuesta válida. Ante una caída de transporte o la ausencia del capability Marketplace, `submitListing` y `submitPurchase` conservan la orden firmada en una cola volátil de red (`getPendingOperations`) con la razón correspondiente; `retryPending` solo la retira después de un acuse válido. No se presenta como transmitida, no se agrega al libro de órdenes local, no obtiene la semilla ni accede a `localStorage` privado. `assetProtocolSupported` y `marketplaceProtocolSupported` son capacidades distintas: la primera puede estar activa para consultar Assets nativos de WebDollar2 mientras la segunda sigue en `false`; en ese caso la UI permite revisar y encolar después de confirmación, pero no publica ni compra.
