...
La prueba de confirmación en Chrome modifica el objeto pasado al hook tras abrir la revisión e intenta un clic sintético. Comprueba que no hay envío antes del clic de navegador y que el paquete enviado conserva 90 WEBD al destinatario para la revisión de 100 WEBD, con débito 109.686. Las seis pruebas de navegador pasan con esta regresión incluida.

Corrección posterior del feed: el cliente reconstruye ahora los eventos Socket.IO con adjuntos binarios antes de publicar `head/new-block`. Las pruebas verifican adjuntos entrantes, framing binario saliente, órdenes Marketplace y solicitudes anidadas del pool: 24 pruebas de núcleo aprobadas. Se elimina el temporizador anterior antes de cada refresco para evitar sondeos duplicados.

- 24 pruebas de núcleo: parsing .webd oficial, integridad WIF, consistencia de claves/dirección, enteros exactos, firmas Ed25519 comprobadas independientemente con Node crypto, espejo Mainnet marcado de solo lectura, transmisión elegible con acuse RPC exacto, bloqueo de política, reservas de nonce, recepción sin crédito ficticio, aislamiento de plugins, lectura de Assets nativos WebDollar2, órdenes Marketplace, rechazo de broadcast falso cuando Mainnet no anuncia Marketplace y paquetes Socket.IO nativos.
- Seis pruebas en Chrome for Testing: importación y revisión 100→90, gate de transmisión que solo acepta el acuse después de Confirmar Envío, respuesta de UI tras fallo de minería, diseño sin desbordamiento horizontal a 1440/375/320 px, intercambio QR entre dos instancias móviles offline, rechazo de recepción repetida, reclamo sujeto a confirmación/política, recarga offline, cambio de idioma/tema y revisión/transmisión Marketplace.
...
- El chequeo estático comprueba artefactos requeridos, sintaxis, iconos y que localStorage solo contenga la preferencia de idioma, nunca claves, saldos o nonces.
...
- `npm run probe:mining` (con `WEBD_MINING_ADDRESS` pública) confirmó el saludo firmado del pool Timi, entregó trabajo PoS de Mainnet en la altura `5972189` y validó el objetivo de 32 bytes; `transactionsBroadcast: 0` y `privateWalletAccessed: false`. La salida es diagnóstica y no se versiona.
...
- El APK debug v3.0 recompilado tiene SHA-256 `056C61F3657416187A8EFA5D95F2D57F99B7308D7434C3C97D4210DC7C88478D`. La compilación y las seis pruebas de navegador pasan; la recompilación incluye cinco idiomas, tema persistente, AES-GCM, mensajería, métricas de minería, NFC con fallback QR, protección de vales offline v2, lectura de Assets WebDollar2, el adaptador Marketplace Mainnet con capability gate y cola de órdenes firmadas pendientes de red.
- El build local también produce `build/release/WebDollar-wallet-debug.aab`, SHA-256 `BDA1CDA1082917A859E1E492E15C0A3299921414CA6582EB4131A3B592F329EC`.
- La suite de protocolo contiene 24 pruebas, incluyendo roundtrip AES-GCM, rechazo de replay por nonce/voucher, lectura de Assets desde las APIs nativas `/account` y `/asset` de WebDollar2, conservación de una orden Marketplace firmada cuando no hay nodo y rechazo explícito de cualquier POST cuando Mainnet no anuncia Marketplace. `npm run probe:mining` volvió a completar el handshake Mainnet de solo lectura con trabajo PoS real, sin acceder a claves privadas ni transmitir transacciones.
- npm audit informa cero vulnerabilidades en las dependencias instaladas.
- La conexión directa de solo lectura a `https://pool.timi.ro/marketplace/capabilities` y `https://webdollar.cloudns.nz/api/marketplace/capabilities` devolvió HTTP 404; la conexión Mainnet de `https://pool.timi.ro` fue reconocida con altura `5972771`. El módulo no interpreta 404 como soporte: muestra el saldo WEBD real, permite revisión humana y conserva una orden firmada como `pending-network` sin publicarla ni comprar, hasta que un nodo anuncie `webdollar-marketplace-v1`. La prueba adicional de contrato WebDollar2 demuestra la lectura de tokens cuando `/account` y `/asset` existen, sin habilitar Marketplace por accidente.
...
| Clave privada local, fuera de red/persistencia | campos privados, localStorage limitado a idioma, pruebas | Comprobado dentro del flujo implementado |
...
| Conexión activa Mainnet | `pool.timi.ro` responde identidad y `/top`; la salida de la sonda no se versiona | Comprobado |
...
| Marketplace sin simulación | capability gate, consultas Assets/listados, firma humana y POST de venta/compra; nodos actuales devuelven 404 | Implementado y bloqueado de forma segura hasta protocolo Mainnet |
...
6. WebDollar2 sigue en desarrollo y sus nodos Mainnet actuales no anuncian el capability Marketplace; no existe una operación real de token/listado que pueda transmitirse honestamente desde esta entrega.

El núcleo Mainnet y la transmisión supervisada están comprobados con el hash documentado. La integración del motor híbrido PoW/PoS y la entrega de trabajo Mainnet están comprobadas, pero no se forzó una solución/recompensa de minería. Permanecen pendientes una reclamación Ecash que liquide fondos reales, una prueba en dispositivo Android físico y la publicación oficial del protocolo Marketplace/Assets para poder transmitir ofertas y compras de tokens reales. El APK debug local fue generado y su hash está documentado en README.md.
