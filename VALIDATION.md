...
- La conexión directa de solo lectura a `https://pool.timi.ro/marketplace/capabilities` y `https://webdollar.cloudns.nz/api/marketplace/capabilities` devolvió HTTP 404; la conexión Mainnet de `https://pool.timi.ro` fue reconocida con altura `5972752`. El módulo no interpreta 404 como soporte: muestra el saldo WEBD real, permite revisión humana y conserva una orden firmada como `pending-network` sin publicarla ni comprar, hasta que un nodo anuncie `webdollar-marketplace-v1`. La prueba adicional de contrato WebDollar2 demuestra la lectura de tokens cuando `/account` y `/asset` existen, sin habilitar Marketplace por accidente.
