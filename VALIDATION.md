...
Corrección posterior del feed: el cliente reconstruye ahora los eventos Socket.IO con adjuntos binarios antes de publicar `head/new-block`. Las pruebas verifican adjuntos entrantes, framing binario saliente, órdenes Marketplace y solicitudes anidadas del pool: 23 pruebas de núcleo aprobadas. Se elimina el temporizador anterior antes de cada refresco para evitar sondeos duplicados.

- 23 pruebas de núcleo: parsing .webd oficial, integridad WIF, consistencia de claves/dirección, enteros exactos, firmas Ed25519 comprobadas independientemente con Node crypto, espejo Mainnet marcado de solo lectura, transmisión elegible con acuse RPC exacto, bloqueo de política, reservas de nonce, recepción sin crédito ficticio, aislamiento de plugins, lectura de Assets nativos WebDollar2, órdenes Marketplace y paquetes Socket.IO nativos.
...
- La suite de protocolo contiene 23 pruebas, incluyendo roundtrip AES-GCM, rechazo de replay por nonce/voucher, lectura de Assets desde las APIs nativas `/account` y `/asset` de WebDollar2 y conservación de una orden Marketplace firmada cuando no hay nodo. `npm run probe:mining` volvió a completar el handshake Mainnet de solo lectura con trabajo PoS real, sin acceder a claves privadas ni transmitir transacciones.
- npm audit informa cero vulnerabilidades en las dependencias instaladas.
- La conexión directa de solo lectura a `https://pool.timi.ro/marketplace/capabilities` y `https://webdollar.cloudns.nz/api/marketplace/capabilities` devolvió HTTP 404; la conexión Mainnet de `https://pool.timi.ro` fue reconocida con altura `5972766`. El módulo no interpreta 404 como soporte: muestra el saldo WEBD real, permite revisión humana y conserva una orden firmada como `pending-network` sin publicarla ni comprar, hasta que un nodo anuncie `webdollar-marketplace-v1`. La prueba adicional de contrato WebDollar2 demuestra la lectura de tokens cuando `/account` y `/asset` existen, sin habilitar Marketplace por accidente.
