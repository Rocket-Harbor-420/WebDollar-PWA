...
Corrección posterior del feed: el cliente reconstruye ahora los eventos Socket.IO con adjuntos binarios antes de publicar `head/new-block`. Las pruebas verifican adjuntos entrantes, framing binario saliente, órdenes Marketplace y solicitudes anidadas del pool: 21 pruebas de núcleo aprobadas. Se elimina el temporizador anterior antes de cada refresco para evitar sondeos duplicados.

- 21 pruebas de núcleo: parsing .webd oficial, integridad WIF, consistencia de claves/dirección, enteros exactos, firmas Ed25519 comprobadas independientemente con Node crypto, espejo Mainnet marcado de solo lectura, transmisión elegible con acuse RPC exacto, bloqueo de política, reservas de nonce, recepción sin crédito ficticio, aislamiento de plugins, órdenes Marketplace y paquetes Socket.IO nativos.
...
- El APK debug v3.0 recompilado tiene SHA-256 `5BC79C002A16BD4135A550D492EDE5EA09826CABB71842DD3FC06EE38A68A358`. La compilación y las seis pruebas de navegador pasan; la recompilación incluye cinco idiomas, tema persistente, AES-GCM, mensajería, métricas de minería, NFC con fallback QR, protección de vales offline v2, el adaptador Marketplace Mainnet con capability gate y cola de órdenes firmadas pendientes de red.
- El build local también produce `build/release/WebDollar-wallet-debug.aab`, SHA-256 `D17DAABEA33FA86E9FB686ED1CC931773B81D3617F31DDBA623DCBAE540D9778`.
- La suite de protocolo contiene 22 pruebas, incluyendo roundtrip AES-GCM, rechazo de replay por nonce/voucher y conservación de una orden Marketplace firmada cuando no hay nodo. `npm run probe:mining` volvió a completar el handshake Mainnet de solo lectura con trabajo PoS real, sin acceder a claves privadas ni transmitir transacciones.
- npm audit informa cero vulnerabilidades en las dependencias instaladas.
- La conexión directa de solo lectura a `https://pool.timi.ro/marketplace/capabilities` y `https://webdollar.cloudns.nz/api/marketplace/capabilities` devolvió HTTP 404. El módulo no interpreta ese resultado como soporte: muestra el saldo WEBD real y bloquea firma/compra/listado hasta que un nodo anuncie `webdollar-marketplace-v1`.
