...
export interface IMarketplacePendingOperation {id:string;kind:'listing'|'purchase';order:MarketplaceOrder;queuedAt:string;status:'pending-network';reason:'marketplace-protocol-unavailable'|'transport-unavailable';}
export interface IMarketplaceModule {connect(endpoint?:string):Promise<{connected:boolean;endpoint:string|null;height?:number;assetProtocolSupported:boolean;marketplaceProtocolSupported:boolean;assetApiFlavor?:string|null;message:string}>;connectToMarketplace(endpoint?:string):Promise<{connected:boolean;endpoint:string|null;height?:number;assetProtocolSupported:boolean;marketplaceProtocolSupported:boolean;assetApiFlavor?:string|null;message:string}>;fetchAssets(address?:string):Promise<{address:string;assets:Array<{id:string;symbol:string;name:string;balance:string;native:boolean;decimals:number}>;tokenAssetsAvailable:boolean;assetProtocolSupported:boolean;message:string}>;listAssetForSale(assetId:string,amount:string|number,price:string|number):{reviewRequired:true;operation:'list';assetId:string;amount:string;price:string};buyAsset(listingId:string):{reviewRequired:true;operation:'buy';listingId:string;assetId:string;amount:string;price:string;seller:string};getListings():Promise<Array<{id:string;listingId:string;assetId:string;amount:string;price:string;seller:string;status:string}>>;submitListing(order:MarketplaceOrder):Promise<unknown>;submitPurchase(order:MarketplaceOrder):Promise<unknown>;getPendingOperations():IMarketplacePendingOperation[];retryPending():Promise<{attempted:number;transmitted:number;pending:IMarketplacePendingOperation[]}>;}
